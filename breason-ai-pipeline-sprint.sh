...
# Breason AI pipeline sprint patch
# Run from the repository root in GitHub Codespaces.

ROOT="$(pwd)"
ROUTE="apps/breason/app/api/resonance-trends/route.ts"
SHARED_PIPELINE="packages/shared/src/ai-pipeline.ts"
SHARED_INDEX="packages/shared/src/index.ts"
TYPES="packages/types/src/index.ts"
ENV_EXAMPLE=".env.example"
BACKUP_DIR=".breason-backup-$(date +%Y%m%d-%H%M%S)"

if [ ! -f "$ROUTE" ] || [ ! -f "$SHARED_INDEX" ] || [ ! -f "$TYPES" ]; then
  echo "Run this script from the Breason monorepo root. Required files were not found."
  exit 1
fi

mkdir -p "$BACKUP_DIR"
cp "$ROUTE" "$BACKUP_DIR/route.ts"
cp "$SHARED_INDEX" "$BACKUP_DIR/index.ts"
cp "$TYPES" "$BACKUP_DIR/types-index.ts"
[ -f "$ENV_EXAMPLE" ] && cp "$ENV_EXAMPLE" "$BACKUP_DIR/.env.example"

echo "Backup saved to $BACKUP_DIR"

cat > "$SHARED_PIPELINE" <<'TS'
import "server-only";
...
route_path = Path("apps/breason/app/api/resonance-trends/route.ts")
route = route_path.read_text()
route = route.replace('import { GoogleGenerativeAI } from "@google/generative-ai";\n', 'import { callFreeAiJson } from "@breason/shared";\n')
route = re.sub(r'const genAI = new GoogleGenerativeAI[\s\S]*?// ── Утилиты', '// ── Утилиты', route, count=1)
route = re.sub(
    r'// ── AI fallbacks[\s\S]*?// ════════════════════════════════════════════════════════════════════════════',
...
if 'export * from "./ai-pipeline";' not in index:
    index = index.replace('export * from "./ai";\n', 'export * from "./ai";\nexport * from "./ai-pipeline";\n')
index_path.write_text(index)

types_path = Path("packages/types/src/index.ts")
types = types_path.read_text()
types = re.sub(
    r"export type AIProvider = [^;]+;",
...
block = """
# AI Pipeline - free-tier first
# OpenAI runs first only when these guards are explicitly enabled.
# Enable only after the OpenAI org owner opts into input/output sharing and the dashboard says the project is enrolled for complimentary daily tokens.
OPENAI_ENABLED=true
OPENAI_FREE_ONLY=true
OPENAI_DATA_SHARING_CONFIRMED=false
...
GROQ_COMPOUND_TIMEOUT_MS=28000
GROQ_MAX_OUTPUT_TOKENS=2600

OPENROUTER_ENABLED=true
OPENROUTER_MODEL=openai/gpt-oss-120b:free
OPENROUTER_TIMEOUT_MS=28000
OPENROUTER_MAX_OUTPUT_TOKENS=2600

AI_TEMPERATURE=0.35
AI_CIRCUIT_FAIL_THRESHOLD=2
AI_CIRCUIT_RESET_MS=60000
""".strip() + "\n"
for key in [
    "OPENAI_ENABLED=", "OPENAI_FREE_ONLY=", "OPENAI_DATA_SHARING_CONFIRMED=", "OPENAI_MODEL=", "OPENAI_DAILY_TOKEN_LIMIT=",
    "GEMINI_MODEL=", "GROQ_QWEN_MODEL=", "GROQ_LLAMA_MODEL=", "GROQ_COMPOUND_MODEL=", "OPENROUTER_MODEL=",
]:
...
# The Google SDK is no longer used by the resonance API after this patch. Remove only if nothing else imports it.
if ! grep -R "@google/generative-ai\|GoogleGenerativeAI" -n apps packages --exclude-dir=node_modules | grep -v package.json >/tmp/breason-google-sdk-uses.txt; then
  npm uninstall @google/generative-ai -w apps/breason || true
else
  echo "Keeping @google/generative-ai because it is still imported:"
  cat /tmp/breason-google-sdk-uses.txt
fi
...
echo ""
echo "Patch complete. Review changes with: git diff"
echo "OpenAI will be skipped until OPENAI_DATA_SHARING_CONFIRMED=true is set in your deployment environment."

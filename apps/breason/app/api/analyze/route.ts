...
import { analyzeMarketing, logger } from "@breason/shared";
...
export async function POST(request: Request): Promise<Response> {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const result: AnalyzeResult = await analyzeMarketing(text, market, requestId);
  ...
  try {
    const result = await analyzeMarketing(text, market, requestId);

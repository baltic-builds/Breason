...
async function getBestGeminiModel(): Promise<string> {
  ...
  try {
    ...
    // Берём все flash-модели (кроме thinking — они медленные),
    // которые поддерживают generateContent
    const flashModels = models
      ...
      .sort((a, b) => {
        // Сортируем по версии: сначала новее
    ...
    // Предпочитаем lite-варианты — они быстрее и дешевле
...
// ── Главный AI-вызов с каскадным fallback ────────────────────────────────────
// Ключевые фиксы:
// 1. Promise.race с таймаутом 20s — Gemini не повиснет на 503 на всё время maxDuration
// 2. Кэш сбрасывается ТОЛЬКО при ошибках модели (404/not found), но НЕ при 503
//    Это важно: при 503 модель существует, просто перегружена — не нужно её переискивать
// 3. Подробное логирование каждого шага — видно что происходит в Vercel logs
async function callAI(prompt: string): Promise<any> {
  ...
  try {
    ...
    // Таймаут 20 секунд — оставляем время для fallback в рамках maxDuration=60
    ...
    // Сбрасываем кэш только если модель не найдена — при 503 кэш сохраняем
    const isModelError = msg.includes('404') || msg.includes('not found') || msg.includes('does not exist');
    if (isModelError) {
...
Лёгкий скептицизм по отношению к западному хайпу вокруг искусственного интеллекта и цифровой трансформации — нужно приземлять в конкретику.
    `.trim(),
    fewShotPass: `
...
Почему провал: нет цифр, нет польских клиентов, нет цены. Хайп без доказательств. «Businesses worldwide» звучит как «не для Польши».
    `.trim(),
    localAnchors: [
...
Бюрократия и сложность интеграции — главные страхи.
    `.trim(),
    decisionLogic: `
...
Неформальное обращение работает лучше в большинстве деловых контекстов.
...
Почему работает: приветствие → понимание локальной боли → бразильские кейсы → поддержка в мессенджере → лёгкий призыв к действию без давления.
    `.trim(),
    fewShotFail: `
...
Почему провал: английский язык, формальное обращение, язык иностранца, который не понимает Бразилию. Нет тепла, нет отношений, нет простоты.
    `.trim(),
    localAnchors: [
...
ВАЖНО: Весь твой ответ — строго на русском языке. Никаких иностранных слов, терминов и аббревиатур в полях JSON, кроме полей suggested и suggested_local, которые требуют иностранного языка по условию задачи.
...
СИЛЬНЫЕ МЕСТНЫЕ ФОРМУЛИРОВКИ:
${p.powerPhrases.join(' | ')}

АВТОРИТЕТНЫЕ ИСТОЧНИКИ РЫНКА: ${p.mediaSources}
...
Шаг 5 — Конкретные улучшения для трёх блоков с применением культурных якорей.
</процесс_мышления>

ШКАЛА ТОНА (целые числа от −5 до +5):
официальный_разговорный: −5 = устав компании, +5 = переписка с другом
...
глобальный_местный: −5 = глобальный шаблон, +5 = написано местным для местных

ИДЕАЛЬНЫЙ ПРОФИЛЬ ТОНА ДЛЯ ${p.labelRu.toUpperCase()}: ${p.idealToneProfile}

КРИТИЧЕСКИЕ ПРАВИЛА ВЫВОДА — НАРУШЕНИЕ НЕДОПУСТИМО:
1. Все поля JSON — ТОЛЬКО на русском (кроме suggested и suggested_local).
2. suggested_local — строго на ${p.language}.
...
7. Вердикт «ПРОШЁЛ» — только если текст написан как местным автором.

Ответь ТОЛЬКО валидным JSON:
{
  "вердикт": "ПРОШЁЛ" | "ПОДОЗРИТЕЛЬНЫЙ" | "ЧУЖОЙ",
  "причина_вердикта": "Одно точное предложение на русском",
  "реакция_покупателя": "Два-три предложения на русском от лица реального покупателя — что он подумает прочитав текст",
  "оценка_шаблонности": <целое число 0–100>,
  "шаблонные_фразы": ["точная фраза из текста", "ещё одна фраза"],
  "упущенные_якоря": ["якорь который упущен и мог бы усилить текст"],
  "тон": {
  ...
  "разрыв_тона": "Одно предложение на русском — насколько текущий тон отличается от идеального",
  "недостающие_сигналы_доверия": ["чего не хватает для доверия — на русском"],
  "контекст_рынка": "Одно предложение на русском о текущем деловом контексте в ${p.labelRu}",
  "правки": [
    {
      "блок": "Заголовок",
      ...
      "обоснование": "Почему новый вариант работает лучше — на русском"
    },
    {
    ...
    },
    {
      ...
      "обоснование": "Почему новый вариант убеждает лучше — на русском"
    }
  ],
  "итоговый_текст": "Полная переработанная версия на английском, нативная для ${p.labelRu}. Только обычный текст.",
  "итоговый_текст_локальный": "Полная переработанная версия на ${p.language}. Только обычный текст."
}
  `.trim();
}

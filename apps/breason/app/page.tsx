...
const MARKETS: Record<MarketKey, { labelRu: string; flag: string; flagCdn: string; desc: string }> = {
...
// Кросс-платформенный флаг: img из flagcdn.com, fallback — эмодзи
// Windows Chrome/Edge не рендерит региональные индикаторы — img решает проблему
const FlagImg = ({ code, emoji, size = 28 }: { code: string; emoji: string; size?: number }) => (
  <img
    ...
    alt={emoji}
    style={{ objectFit: "cover", borderRadius: 3, display: "inline-block", verticalAlign: "middle" }}
    onError={(e) => {
      // Fallback: скрываем img, показываем эмодзи через data-attr
      ...
      <div className="market-selector">
        {(Object.entries(MARKETS) as [MarketKey, typeof MARKETS[MarketKey]][]).map(([key, m]) => (
          <button key={key} className={`market-card ${market === key ? "active" : ""}`} onClick={() => setMarket(key)}>
            <span className="market-card-flag"><FlagImg code={m.flagCdn} emoji={m.flag} size={32} /></span>

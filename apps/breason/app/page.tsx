...
const stepSubs:   Record<Step, string> = { search: "Тренды рынка", evaluate: "Анализ текста", improve: "AI-рефайн" };
...
function SearchStep({ onSendToImprove, onSendToEvaluate }: {
  ...
  async function findTrends() {
    ...
    try {
      ...
      setTrends(((await res.json()) as ResonanceTrendsResponse).trends);
      ...
      {!result ? (
        ...
        <>
          ...
          <button className="btn-next" onClick={() => onSendToImprove(text)}>
            <div><span className="btn-next-label">Улучшить этот текст</span><span className="btn-next-sub">Передать в AI-рефайн с учётом результатов анализа</span></div>
    ...
    <>
      <div className="hero">
        <div>
          ...
          <p className="hero-sub">AI-рефайн с учётом рынка, tone of voice и маркетинговых задач.</p>
        </div>

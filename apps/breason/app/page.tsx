    ...
    <div className="split">
      <div className="stack" style={{ position: 'sticky', top: 80 }}>
        <div className="card">
          ...
          {selectedTrend ? (
            <div className="context-pill">🎯 Тренд: {selectedTrend.length > 30 ? selectedTrend.slice(0,30) + "..." : selectedTrend} <span style={{cursor:'pointer', marginLeft: 4}} onClick={()=>setSelectedTrend("")}>✕</span></div>
          ) : (
    ...
    <div className="split">
      <div className="stack" style={{ position: 'sticky', top: 80 }}>
        <div className="card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
            ...
            <button className={`btn-action ${styleModifier === "professional" ? "active" : ""}`} onClick={() => setStyleModifier("professional")}>💼 Усилить экспертность</button>
      ...
      <div className="stack">
        ...
        {!loading && !improveResult && !error && (
          <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--t3)' }}>
            <div style={{ fontSize: 40, opacity: 0.2, marginBottom: 16 }}>✨</div>
            <p>Выберите стиль и нажмите кнопку. ИИ перепишет ваш текст, учитывая контекст рынка и предыдущие ошибки.</p>
          </div>
        )}

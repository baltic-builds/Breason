...
function MarketPicker({ market, onChange }: { market: MarketKey; onChange: (m: MarketKey) => void }) {
  return (
    <div className="market-row">
      {(Object.keys(MARKETS) as MarketKey[]).map((k) => (
        <button key={k} className={`mkt${market === k ? " sel" : ""}`} onClick={() => onChange(k)}>
      ...
      {trends.map((t) => (
        <div className="trend-card" key={t.title}>
          ...
          <div className="trend-actions">
            ...
            <button className="btn-ghost" onClick={() => onSendToEvaluate(`${t.title}. ${t.insight}`)}>
              Проверить →
            </button>
            <button className="btn-ghost" onClick={() => onSendToImprove(`${t.title}. ${t.insight}`)}>
              Улучшить →
            </button>
          </div>
              ...
              {!generated ? (
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                    <button
                      className="btn-ghost"
                      onClick={() => onSendToEvaluate(`${generated.headline}\n\n${generated.body}`)}
                    >
                      Проверить →
                    </button>
                    <button
                      className="btn-ghost"
                      onClick={() => onSendToImprove(`${generated.headline}\n\n${generated.body}`)}
                    >
                      Улучшить →
                    </button>
                  </div>
...
function EvaluateStep({
  ...
  useEffect(() => {
    if (initialText) { setText(initialText); setResult(null); }
  }, [initialText]);
  ...
  const verdictCfg = result
    ? ({ PASS: { cls: "v-pass", icon: "✓", label: "Звучит нативно" }, SUSPICIOUS: { cls: "v-sus", icon: "~", label: "Немного импортно" }, FOREIGN: { cls: "v-for", icon: "✕", label: "Читается как перевод" } }[result.verdict])
    : null;
        ...
        <>
          ...
          <div className="card">
            ...
            <div className="score-bar-wrap">
              <div className="score-bar-fill" style={{ width: `${generic}%`, background: scoreColor }} />
            </div>
          ...
          <div className="card">
            ...
            <p style={{ fontSize: 13, color: "var(--t2)", lineHeight: 1.65 }}>{result.marketTension}</p>
          </div>

          <div className="card">
            <div className="card-label">Сигналы доверия</div>
            <div className="list-items">
              {result.strengths.map((x) => <div className="list-item" key={x}><span className="bullet b-green" />{x}</div>)}
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="list-items">
              {result.risks.map((x) => <div className="list-item" key={x}><span className="bullet b-red" />{x}</div>)}
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="list-items">
              {result.suggestions.map((x) => <div className="list-item" key={x}><span className="bullet b-blue" />{x}</div>)}
            </div>
          </div>
          ...
          <button className="btn-next" onClick={() => onSendToImprove(text)}>
            <div>
              ...
              <span className="btn-next-sub">Передать в AI-рефайн с учётом результатов анализа</span>
            </div>
            <span className="btn-next-arrow">→</span>
          </button>

          <div style={{ marginTop: 8 }}>
            <button className="btn-ghost" onClick={() => setResult(null)}>← Новый анализ</button>
          </div>
        </>
      )}
    </>
...
function ImproveStep({ initialText }: { initialText: string }) {
  ...
  useEffect(() => {
    if (initialText) { setInputText(initialText); setResult(""); setMeta(null); }
  }, [initialText]);

  useEffect(() => {
    if (isLoading && outRef.current) outRef.current.scrollTop = outRef.current.scrollHeight;
  }, [result, isLoading]);
  ...
  async function run() {
    ...
    setIsLoading(true); setResult(""); setMeta(null); setError("");
    const t0 = Date.now();

    const payload: ReDuckProcessRequest = {
      text: inputText, providerId: provider, modelId: "latest", promptVersion: prompt,
    };
    ...
    try {
      ...
      setMeta({
        provider: provider as any, promptVersion: prompt,
        latencyMs: Date.now() - t0, requestedAt: new Date().toISOString(), costUsd: 0.0008,
      });
    } catch (e: any) {
            ...
            {!result && !isLoading ? (
              ...
              <>
                <div className="result-text">
                  {result}{isLoading && <span className="cursor" />}
                </div>
                {meta && !isLoading && (
                  <div className="meta-row">
        ...
        <div className="main">
          ...
          <div className="step-bar">
            {(["search", "evaluate", "improve"] as Step[]).map((s) => (
              <div key={s} className={`step-seg${step === s ? " active" : ""}`} />
            ))}
          </div>

          <div className="content">
            {step === "search"   && (
              <SearchStep onSendToImprove={goImprove} onSendToEvaluate={goEvaluate} />
            )}
            {step === "evaluate" && (
              <EvaluateStep initialText={evaluateText} onSendToImprove={goImprove} />
            )}
            {step === "improve"  && (
              <ImproveStep initialText={improveText} />
            )}
          </div>

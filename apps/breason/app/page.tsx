...
function SearchStep({ onSendToImprove, onSendToEvaluate }: {
  ...
  async function findTrends() {
    setLoading(true); setTrends([]); setGenerated(null); setGenFor(null); setAnalystNote(null);
    try {
    ...
    <>
      <div className="hero">
        <div>
          ...
          <p className="hero-sub">Живые B2B-тренды с генерацией контента и проверкой соответствия аудитории.</p>
        </div>
      ...
      {analystNote && (
        <div style={{ background: "var(--s1)", border: "1px solid var(--b1)", borderLeft: "2px solid rgba(200,241,53,.4)", borderRadius: "var(--r2)", padding: "10px 14px", marginTop: 12, fontSize: 12.5, color: "var(--t2)", lineHeight: 1.6 }}>
          <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: ".07em", textTransform: "uppercase", color: "var(--t3)", display: "block", marginBottom: 4 }}>Настроение рынка · 2026</span>
          {analystNote}
        </div>
      )}
      {trends.map((t) => (
        <div className="trend-card" key={t.title}>
          ...
          <div className="trend-badges">
            ...
            {t.marketTension && <span className="tag tag-blue">{t.marketTension}</span>}
          </div>
          {(t as any).narrative_hook && (
            <p style={{ fontSize: 13.5, color: "var(--t1)", lineHeight: 1.6, marginBottom: 10, fontStyle: "italic" }}>
              «{(t as any).narrative_hook}»
            </p>
          )}
          <p className="trend-insight">{t.insight}</p>
          {(t as any).why_now && (
            <div style={{ marginTop: 8, padding: "8px 12px", background: "var(--s2)", borderRadius: "var(--r2)", borderLeft: "2px solid var(--lime)" }}>
              <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: ".07em", textTransform: "uppercase", color: "var(--t3)" }}>Почему сейчас</span>
              <p style={{ fontSize: 12.5, color: "var(--t2)", lineHeight: 1.6, marginTop: 4 }}>{(t as any).why_now}</p>
            </div>
          )}
          <div className="trend-actions" style={{ marginTop: 14 }}>
            <button className="btn-primary" style={{ height: 34, fontSize: 12 }} onClick={() => generateContent(t)}>Сгенерировать контент</button>
            <button className="btn-ghost" onClick={() => onSendToEvaluate(`${t.title}. ${t.insight}`)}>Проверить →</button>
            <button className="btn-ghost" onClick={() => onSendToImprove(`${t.title}. ${t.insight}`)}>Улучшить →</button>
          </div>
          {genFor === t.title && (
            <div className="gen-card">
              {!generated ? <div style={{ fontSize: 13, color: "var(--t2)" }}>Генерируем…</div> : (
                <>

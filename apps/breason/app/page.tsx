...
:root{
  ...
  --lime:#C8F135;--lime2:#A8D920;--blue:#3B82F6;--green:#22C55E;--amber:#F59E0B;--red:#EF4444;
  --font:'DM Sans',system-ui,sans-serif;--disp:'Syne',system-ui,sans-serif;--r:12px;--r2:8px;
}
...
.improve-shell{display:flex;min-height:520px;border:1px solid var(--b1);border-radius:var(--r);overflow:hidden}
...
function EvaluateStep({ initialText, onSendToImprove }: { initialText: string; onSendToImprove: (t: string) => void }) {
  ...
  const verdictCfg = result ? ({ PASS: { cls: "v-pass", icon: "✓", label: "Звучит нативно" }, SUSPICIOUS: { cls: "v-sus", icon: "~", label: "Немного импортно" }, FOREIGN: { cls: "v-for", icon: "✕", label: "Читается как перевод" } }[result.verdict as "PASS" | "SUSPICIOUS" | "FOREIGN"]) : null;
        ...
        <div className="improve-left">
          ...
          <div className="panel-body">
            <div>
              ...
              <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Вставьте текст, который хотите улучшить…" rows={8} />
            ...
            <button className="btn-primary full" onClick={run} disabled={isLoading || !inputText.trim()}>
              {isLoading ? "Улучшаем…" : "Улучшить текст →"}
            </button>
            {isLoading && <button className="btn-stop" onClick={() => abortRef.current?.abort()}>Остановить генерацию</button>}
          </div>
        </div>
        <div className="improve-right">
          <div className="out-hd">
            <span className="out-hd-title">{result ? "Улучшенный текст" : "Результат"}</span>
            {result && (
          ...
          {error && <div className="err" style={{ margin: "16px 20px 0" }}>{error}</div>}
          <div className="out-body" ref={outRef}>
            {!result && !isLoading ? (
              <div className="empty-state">
                <div className="empty-icon">✦</div>
                <div className="empty-title">Готов к работе</div>
                <div className="empty-sub">Вставьте текст слева и нажмите «Улучшить» — результат появится здесь.</div>
              </div>
            ) : (
              <>
                ...
                {meta && !isLoading && (
                  <div className="meta-row">
                    ...
                    <div className="meta-item"><span>Промпт </span>{meta.promptVersion}</div>
    ...
    <>
      ...
      <div className="shell">
        <aside className="sidebar">
          ...
          <div className="sb-foot">Breason · v0.5</div>
        </aside>
        ...
        <div className="main">
          ...
          <div className="bottom-bar">
            <input
              className="bar-inp"
              placeholder={step === "search" ? "Введите тему для поиска трендов…" : step === "evaluate" ? "Вставьте текст для быстрого анализа…" : "Вставьте текст для улучшения…"}
              value={barText}
              onChange={(e) => setBarText(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleBarSubmit()}
            />
            <button className="bar-send" onClick={handleBarSubmit}>→</button>
          </div>
        </div>
      </div>
    </>
